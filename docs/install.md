...
```
oc get csv servicemeshoperator.v2.6.4 -n openshift-operators -o json | jq '.status.phase'
```
...
spec:
  ...
  source: redhat-operators
  sourceNamespace: openshift-marketplace
  startingCSV: serverless-operator.v1.34.1
EOF
```
verify subscription "Succeeded" in install

```
oc get csv serverless-operator.v1.34.1 -n openshift-serverless -o json | jq '.status.phase'
```
...
verify subscription "Succeeded" in install
```
oc get csv opendatahub-operator.v2.22.0 -n openshift-operators -o json | jq '.status.phase'
```
